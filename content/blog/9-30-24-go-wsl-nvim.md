...
2. Optional tease apart the main `init.lua` file, ie. `nvim\init.lua`. 
	- To do this find different sections of code you would like to pull out of the main file. For example, see this auto format configuration that comes with kickstart. But before you do this be sure to uncomment  `  -- { import = 'custom.plugins' },` in `nvim\init.lua`, this will allow you to tease apart this main init.lua and place the things you tease apart in `nvim\custom\plugin\auto-format.lua` 
```lua
